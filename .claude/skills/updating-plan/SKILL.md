...
`project-documentation/plan.md` is the master plan for vetkit. CLAUDE.md tracks _decisions and conventions_; plan.md tracks _work to be done, in order, with checkboxes_. Drift between code reality and plan.md erodes trust in the doc fast — update it deliberately at the moments below, never reactively or from memory.
...
| Phase 1 backlog order changes                   | Update §2 _and_ note the reason in `CLAUDE.md` §12. Adjust the **Depends on** column if dependencies shifted. |
...
3. **Sync §1 and §2.** A chunk shipped means _both_ sections get checked. If they fall out of sync the doc loses credibility.
...
5. **Commit with the other wrap docs.** Bundle plan.md into the session's single combined docs commit (e.g. `docs(project): wrap chunk N` — owner preference, 2026-06-06). Never mix doc changes with feature code. Follow `.claude/skills/writing-commits/SKILL.md`.
...
| Editing plan.md in the same commit as the feature    | Feature commits should be reviewable on their own. Docs go in the combined wrap-docs commit. |
...
- About to delete a checked-off item — _don't_, it's a record.
...
| "I'll bundle docs + feature into one commit"       | Docs bundle together (owner preference), but never with feature code. |
