...
> **⚠ 2026-06-14 — ARCHITECTURE PIVOT IN PROGRESS (owner decision).** vetkit is migrating **off Sanity onto Supabase** (Postgres + Auth + Storage) and gaining a custom **admin dashboard** (`apps/admin`) for clinic users + a super-admin. This is a **full rebuild** of the data + admin layers; the templates and legacy design are preserved. Several decisions below are being **superseded** — §2.1 (tenancy now one shared Supabase project + RLS), §2.2 (CMS now Supabase, not Sanity), §2.3 (form submissions now persist to a DB **and** email), §3 (stack), §5 (schema → Postgres). See the **2026-06-14 row in §12** and the data-model spec at [`project-documentation/specs/2026-06-14-supabase-data-model.md`](project-documentation/specs/2026-06-14-supabase-data-model.md). Sections are rewritten in place as each rebuild chunk (R1–R10, see plan.md) lands; until then, trust §12 + the spec over the older prose.
...
| 2026-06-14 | **PIVOT: Supabase replaces Sanity + a custom admin dashboard (`apps/admin`); full rebuild.** Supersedes §2.2 (CMS), §2.3 (no-DB forms), parts of §2.1 (tenancy), §3 (stack), §5 (schema), anti-pattern #2. **Tenancy:** ONE shared Supabase project, RLS-isolated by `tenant_id` (deny-by-default + a gating cross-tenant leak test) — public sites keep one-Vercel-project-per-client with build-time `TENANT_ID`; the admin is one multi-tenant deploy where tenant = who's logged in. **Admin:** clinic users (invite-only, `memberships` role owner/staff) + super-admin (`platform_admins`, tenant switcher). **Publish:** draft/publish via a `status` column. **Forms:** contact submissions persist to a `submissions` (leads) table AND email via Resend. **Query layer:** `supabase-js` + `supabase gen types` (NOT Drizzle — it bypasses RLS). **Public read path:** anon key + RLS published-only (Option A). **Preserved:** all `templates/` + legacy design, Tailwind tokens, route tree, SEO helper structure, Resend. **Removed:** `apps/studio`, `lib/sanity/*`, GROQ/typegen/Sanity cache-tags, `packages/sanity-types`. Full data-model + RLS design in [`specs/2026-06-14-supabase-data-model.md`](project-documentation/specs/2026-06-14-supabase-data-model.md). Rebuild roadmap R1–R10 in plan.md. | Owner wants to own the admin UX and grow the backend into operational tables (appointments, patients, leads) a headless CMS can't serve; the "build the admin UI yourself" cost §2.2 avoided is now justified because a custom dashboard is the goal. Shared project + RLS fits a unified admin and future ops tables far better than N isolated projects (the same fan-out the monorepo exists to kill). 2–5 clinics + no content yet = cheapest possible moment to switch. | 2.1, 2.2, 2.3, 3, 5, 9, 11 |
