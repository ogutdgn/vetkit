    ...
    <>
      ...
      <header className="sticky top-0 z-40 border-b border-ink-200 bg-white/90 backdrop-blur">
        <div className="relative mx-auto flex max-w-6xl items-center justify-between gap-4 px-4 py-3 sm:px-6">
          <Link
            href="/"
            className="flex items-center gap-3 focus-visible:outline-2 focus-visible:outline-brand-600"
          >
            {logo.asset ? (
            ...
            <span className="text-lg font-semibold tracking-tight text-ink-900">{clinicName}</span>
          </Link>
            ...
            <ul className="flex items-center gap-6">
              {navItems.map((item) => (
                <li key={item.href}>
                  <Link
                    href={item.href}
                    className="rounded-sm text-sm font-medium text-ink-700 transition hover:text-brand-600 focus-visible:outline-2 focus-visible:outline-offset-4 focus-visible:outline-brand-600"
                  >
                    {item.label}
                  </Link>
                </li>
          ...
          <div className="flex items-center gap-2">
            ...
            <a
              ...
              className="inline-flex rounded-md p-2.5 text-brand-700 hover:bg-brand-50 focus-visible:outline-2 focus-visible:outline-brand-600 sm:hidden"
            >
