...
- `packages/config-eslint/` — shared ESLint flat config preset. Will land alongside the lint setup.
- `packages/config-tailwind/` — shared Tailwind preset (theme tokens, plugins). Lands when we wire up Tailwind v4.
- `packages/sanity-types/` — auto-generated TypeScript types from the Sanity schema (via `sanity-codegen`). Lands once the schema exists.
