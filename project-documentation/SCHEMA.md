...
| `heroSlide`       | `image` (hotspot, alt required), `heading` (string)             | `subheading` (string), `cta` — slides on `siteSettings.heroSlides` (max 5)                                 |
...
| -------------- | --------------------------- | ---------- | ----------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `siteSettings` | —                           | ✓          | —                       | Pinned at `_id: 'siteSettings'` via custom desk structure. Holds identity, hero slides, contact, address, hours, social, emergency banner, footer, SEO defaults, feature flags.                            |
| `service`      | `/hizmetler/[slug]`         | —          | ✓                       | Title, slug, mainImage (alt required), optional icon, short description, rich-text detail, petTypes, serviceLocation (in-clinic/home-call/both), emergencyAvailable, relatedFAQs, pricing, embedded `seo`. |
