...
| OD-5 | **Cache-tag naming convention for `next-sanity` queries** (so the Chunk 13 webhook can selectively `revalidateTag`) | Two viable shapes: `sanity:<type>:<id>` (stable across slug changes — recommended) or `<type>:<slug>:<locale>` (more readable in logs but breaks if a slug is renamed). Convention has to land in Chunk 7 with the first queries; changing it later means touching every query + webhook handler simultaneously. | Chunk 7 (first queries) — practically the first commit of the chunk, since each query that uses a tag bakes the convention into its callers. |
