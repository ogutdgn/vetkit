...
| OD-6 | **i18n: keep field-level `{tr,en}` or simplify to plain single-language fields?** | The 2026-05-26 decision built custom `locale*` types so every text field holds `{ tr, en }`. But CLAUDE.md §3 + anti-pattern #12 say sites are Turkish-only and i18n should wait for a real bilingual client — the owner is reconsidering (2026-05-30). **Option 1** (plain `string` / `text` / `slug` / portable-text fields): simpler editor (one box) and trivial queries (no `$locale` / `coalesce`), but reverses the logged 2026-05-26 decision and reworks the Chunk 4 schema. **Option 2** (status quo): keep `{ tr, en }`, English hidden until a tenant opts in. No Sanity project or content exists yet, so the migration cost is ~zero right now — the cheapest possible moment to decide. | Chunk 7 (first queries) — the field shape decides whether queries project flat strings or `coalesce(field[$locale], …)`. **If Option 1, a Chunk 4 schema-simplification pass runs before Chunk 7.** |
