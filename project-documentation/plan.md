...
- [x] R2 — `apps/admin` shell: `@supabase/ssr` auth, invite-only login, tenant + super-admin resolution
...
| ✓ R2 | `apps/admin` shell — `@supabase/ssr` auth, invite-only login, tenant resolution from membership, super-admin tenant switcher                         | R1         | M    | Shipped 2026-06-14. Next 16 `proxy.ts` (renamed middleware) + async `cookies()` + `getClaims()` per the canonical example; login/protected-shell/tenant-switcher/sign-out; `getActor()` resolves tenant + super-admin via RLS. Bootstrap script seeds the first super-admin + gigi/ovapark tenants. Verified: build/lint, live auth 5/5, runtime proxy 307→/login. |
...
| ~R9  | shadcn/ui init + branded `not-found.tsx`                                                                                                             | R2         | S    | **shadcn/ui init + professional admin UI done 2026-06-15** (pulled forward during R3): base-nova/neutral/Geist, sidebar shell, dashboard stat cards, table + dropdown row actions, card-section forms. Branded `not-found.tsx` still pending.                                                                                                                      |
